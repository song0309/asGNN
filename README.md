...
The code was developed and tested on a cluster with Linux system, which is equipped with 50 CPUs and 512GB memory.
...
The breast cancer tissue data used in this study were obtained from the works [1] and [2]. We followed the data preproessing in the study [1] to extract convolutional features and applied the model in the study [3] to derive morphological features for each tissue image, and provided the prepocessed data for holdout and external validation under the folders `data/holdout` and `data/external` separately. 
...
[1] [Integrating spatial gene expression and breast tumour morphology via deep learning](https://www.nature.com/articles/s41551-020-0578-x), Bryan He, Ludvig Bergenstråhle, Linnea Stenbeck, Abubakar Abid, Alma Andersson, Åke Borg, Jonas Maaskola, Joakim Lundeberg and James Zou, Nature biomedical engineering (2020).
